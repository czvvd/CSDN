...
class NormalizeObjectPose(object):
    ...
    def __call__(self, data):
        ...
        # bbox /= scale
        # ptcloud = (ptcloud - center) / scale
        # # ptcloud = (ptcloud - center) / scale
        # ptcloud = np.dot(ptcloud, [[1, 0, 0], [0, 0, 1], [0, 1, 0]])

        gt_mean = ptcloud.mean(axis=0)
        ptcloud = ptcloud - gt_mean
        pc_L_max = np.max(np.sqrt(np.sum(abs(ptcloud ** 2), axis=-1)))
        ptcloud = ptcloud / pc_L_max
